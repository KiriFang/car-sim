...
costFunc::costFunc(mpcParam* mpp, wtParam* wp, carParam cp, mtnParam mp, float t, bool inc): costparam(*mpp, *wp),
       	cparam(cp), 
	mparam(mp), 
	A(mpp -> Nx),  
        B(mpp -> Nx, mpp -> Nu),
	C(mpp -> Ny, mpp -> Nx, 1),
	preB(mpp -> Np * mpp -> Ny, mpp -> Nu),
        preA(mpp -> Ny, mpp -> Nx + inc * mpp -> Nu),
	preQ(wp -> Q),
	preR(wp -> R),
	H(mpp -> Nu * mpp -> Nc + 1, mpp -> Nu * mpp -> Nc + 1),
	g(mpp -> Np * mpp -> Np, mpp -> Np * mpp -> Np + 1),
	T(t), 
        is_increment(inc) {
...
//		cout << preB.size_r() << " " << preB.size_c() << endl;
